...
def poll_RX():
    ...
    if len(buf) == 11:
        if (
            ...
            if dest == (
                binascii.unhexlify(
                    (buf[3].to_bytes(length=1, byteorder="big", signed=False))
...
def sendreq(TID):
    ser.write(b"$")
    ser.write(
        binascii.hexlify(TID.to_bytes(length=1, byteorder="big", signed=False)).upper()
    )
    ser.write(b"F800000000\r\n")  # GoToBoot
    for x in range(3):
        while ser.inWaiting() > 0:
...
def Enable():
    ...
    ser.write(
        binascii.hexlify(
            addr_msb.to_bytes(length=1, byteorder="big", signed=False)
        ).upper()
    )
    ser.write(
        binascii.hexlify(
            addr_lsb.to_bytes(length=1, byteorder="big", signed=False)
        ).upper()
    )
...
def Send_data(modifier):
    ...
    hx_ID = binascii.hexlify(
        strippedID.to_bytes(length=1, byteorder="big", signed=False)
    ).upper()
    inv_ID = binascii.hexlify(
        (255 - strippedID).to_bytes(length=1, byteorder="big", signed=False)
    ).upper()
    for x in range(256):
        if byte := f.read(1):
            if First:
...
def Erase_data():
    ...
    ser.write(
        binascii.hexlify(
            fblock_msb.to_bytes(length=1, byteorder="big", signed=False)
        ).upper()
    )
    ser.write(
        binascii.hexlify(
            fblock_lsb.to_bytes(length=1, byteorder="big", signed=False)
        ).upper()
    )
    ser.write(b"\r\n")
    if poll_RX() == False:
