...
parser.add_argument(
    "command", choices=["backup", "restore", "verify", "zero", "measure"]
)
parser.add_argument("--start", type=int, default=0, help="start entry to operate on")
parser.add_argument("--stop", type=int, default=155, help="stop entry to operate on")
parser.add_argument(
    "--file",
    default="register_values.json",
...
if args.command == "backup":
        ...
        for i in the_range:
            if str(i) in data:
                ...
                if file_entry != data_entry:
                    print(
                        f"Entry {i} does not match, file = {file_entry} data = {data_entry}"
                    )
                    success = False
            else:
                print(f"Entry {i} not found in {args.file}!")
    ...
    print(
        f"Measuring power offset at frequencies from entry {args.start} to {args.stop} in {args.file}"
    )
    with open(args.file, "r") as f:
        data = json.loads(f.read())
        success = True
