...
def SendCommand(cardId, senderId, register, value):
    port.write(b'$')
    port.write(binascii.hexlify(cardId.to_bytes(length=1, byteorder='big', signed=False).upper()))
    port.write(binascii.hexlify(senderId.to_bytes(length=1, byteorder='big', signed=False).upper()))
    port.write(binascii.hexlify(register.to_bytes(length=2, byteorder='big', signed=False).upper()))
    port.write(binascii.hexlify(value.to_bytes(length=2, byteorder='big', signed=False).upper()))
    port.write(b'\r\n')
...
parser.add_argument('-l', type = valid_frequency, default = 0,
                    ...
                    ', internally converted to integer MHz')
