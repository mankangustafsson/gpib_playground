...
def SendCommand(cardId, senderId, register, value):
    ...
    port.write(
        binascii.hexlify(
            cardId.to_bytes(length=1, byteorder="big", signed=False).upper()
        )
    )
    port.write(
        binascii.hexlify(
            senderId.to_bytes(length=1, byteorder="big", signed=False).upper()
        )
    )
    port.write(
        binascii.hexlify(
            register.to_bytes(length=2, byteorder="big", signed=False).upper()
        )
    )
    port.write(
        binascii.hexlify(
            value.to_bytes(length=2, byteorder="big", signed=False).upper()
        )
    )
    port.write(b"\r\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", default="COM37", metavar="COM-port", help="COM port to use"
    )
    parser.add_argument(
        "-l",
        type=valid_frequency,
        default=0,
